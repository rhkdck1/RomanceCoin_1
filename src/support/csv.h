...
namespace csv {
    ...
    public:
        ...
        // '\r', '\n', or '\r\n'.
        explicit parser(std::istream& input): m_input(input) {
        ...
        parser& quote(char c) noexcept {
            m_quote = c;
            return *this;
        }

        // Change the delimiter character
        parser& delimiter(char c) noexcept {
            m_delimiter = c;
            return *this;
        }

        // Change the terminator character
        parser& terminator(char c) noexcept {
    ...
    public:
        ...
        public:
            ...
            explicit iterator(parser *p, bool end = false): m_parser(p) {
                if (!end) {
                    m_row.reserve(50);
                    m_current_row = 0;
                    next();
