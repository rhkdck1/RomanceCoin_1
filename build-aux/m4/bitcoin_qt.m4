...
AC_DEFUN([MICRO_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build romance-qt GUI (default=auto)])],
    [
     romance_qt_want_version=$withval
     if test "x$romance_qt_want_version" = xyes; then
       romance_qt_force=yes
       romance_qt_want_version=auto
     fi
    ],
    [romance_qt_want_version=auto])
...
dnl Inputs: romance_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If romance_qt_want_version is "auto", check for this version
dnl         first.
